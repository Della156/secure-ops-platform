...
import { Search, Server, CheckCircle, XCircle, AlertTriangle, Shield, Database, Settings } from 'lucide-react';
...
const mockData: Middleware[] = [
  { id: 'MW-001', name: 'Tomcat-01', type: 'Tomcat', version: '9.0.65', ip: '192.168.3.10', status: 'compliant', complianceRate: 92, lastCheck: '2026-06-02 09:00:00' },
  { id: 'MW-002', name: 'Nginx-01', type: 'Nginx', version: '1.24.0', ip: '192.168.3.11', status: 'compliant', complianceRate: 98, lastCheck: '2026-06-02 09:05:00' },
  { id: 'MW-003', name: 'Redis-01', type: 'Redis', version: '7.0.12', ip: '192.168.3.12', status: 'partial', complianceRate: 75, lastCheck: '2026-06-02 09:10:00' },
  { id: 'MW-004', name: 'Kafka-01', type: 'Kafka', version: '3.5.1', ip: '192.168.3.13', status: 'partial', complianceRate: 68, lastCheck: '2026-06-02 09:15:00' },
  { id: 'MW-005', name: 'Elasticsearch-01', type: 'Elasticsearch', version: '8.10.0', ip: '192.168.3.14', status: 'non-compliant', complianceRate: 42, lastCheck: '2026-06-02 09:20:00' },
];

export function MiddlewareBaselineView() {
  const [data] = useState<Middleware[]>(mockData);
  ...
  const getTypeIcon = (type: string) => {
    if (type.includes('Tomcat') || type.includes('Nginx')) return <Settings className="w-4 h-4 text-blue-400" />;
    if (type.includes('Redis')) return <Database className="w-4 h-4 text-red-400" />;
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">中间件基线检查视图</h2>
        <p className="text-sm text-gray-400 mt-1">检查中间件的安全基线配置，确保符合安全标准</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <p className="text-gray-400 text-sm">中间件总数</p>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2A354D]">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">中间件名称</th>
