...
import { Search, Download, RefreshCw, Filter, Eye, Calendar, User, CheckCircle2, XCircle, AlertCircle, FileText, BarChart3, Award, Sparkles, Brain, TrendingUp, Printer } from 'lucide-react';
...
interface Report {
  ...
  qualityScore: number; grade: string; confidence: number; compliancePassed: number; complianceTotal: number;
  score: number;
}

const reports: Report[] = [
  { id: 'JAR-99832', name: '防火墙策略变更辅助报告', jobType: '配置变更', taskNo: 'JAM-99821', submittedAt: '2026-06-03 11:25:00', completedAt: '2026-06-03 12:00:00', duration: '35min', result: 'success', applicant: '李娜', aiSuggestions: 8, appliedSuggestions: 0, improvement: 15, qualityScore: 88, grade: 'A', confidence: 92, compliancePassed: 10, complianceTotal: 12, score: 85 },
  { id: 'JAR-99831', name: 'Oracle 表结构变更辅助报告', jobType: '数据操作', taskNo: 'JAM-99820', submittedAt: '2026-06-03 10:48:18', completedAt: '2026-06-03 11:12:00', duration: '24min', result: 'success', applicant: '王芳', aiSuggestions: 12, appliedSuggestions: 0, improvement: 8, qualityScore: 92, grade: 'A+', confidence: 95, compliancePassed: 18, complianceTotal: 18, score: 92 },
  { id: 'JAR-99830', name: 'Web 集群补丁安装辅助报告', jobType: '漏洞修复', taskNo: 'JAM-99819', submittedAt: '2026-06-03 09:32:00', completedAt: '2026-06-03 10:15:00', duration: '43min', result: 'success', applicant: '陈磊', aiSuggestions: 10, appliedSuggestions: 8, improvement: 22, qualityScore: 82, grade: 'B+', confidence: 86, compliancePassed: 14, complianceTotal: 15, score: 78 },
  { id: 'JAR-99829', name: 'AD 账号调整辅助报告', jobType: '配置变更', taskNo: 'JAM-99818', submittedAt: '2026-06-03 08:15:00', completedAt: '2026-06-03 08:33:00', duration: '18min', result: 'success', applicant: '刘洋', aiSuggestions: 6, appliedSuggestions: 6, improvement: 12, qualityScore: 90, grade: 'A', confidence: 93, compliancePassed: 10, complianceTotal: 10, score: 95 },
  { id: 'JAR-99828', name: '慢 SQL 优化辅助报告', jobType: '日常维护', taskNo: 'JAM-99817', submittedAt: '2026-06-02 22:48:00', completedAt: '2026-06-02 23:20:00', duration: '32min', result: 'failed', applicant: '王芳', aiSuggestions: 8, appliedSuggestions: 0, improvement: 0, qualityScore: 42, grade: 'D', confidence: 65, compliancePassed: 5, complianceTotal: 8, score: 52 },
  { id: 'JAR-99827', name: 'APT 应急处置辅助报告', jobType: '应急处置', taskNo: 'JAM-99816', submittedAt: '2026-06-02 18:30:00', completedAt: '2026-06-02 18:38:00', duration: '8min', result: 'success', applicant: '张伟', aiSuggestions: 15, appliedSuggestions: 14, improvement: 5, qualityScore: 96, grade: 'A+', confidence: 98, compliancePassed: 20, complianceTotal: 20, score: 98 },
];
...
const monthlyTrend = [
  { month: '1月', count: 32, avgScore: 75, improvement: 8 },
  { month: '2月', count: 38, avgScore: 78, improvement: 12 },
  { month: '3月', count: 42, avgScore: 80, improvement: 14 },
  { month: '4月', count: 48, avgScore: 83, improvement: 16 },
  { month: '5月', count: 56, avgScore: 86, improvement: 18 },
  { month: '6月', count: 32, avgScore: 88, improvement: 20 },
];
...
export function JobAssistantTaskReport() {
  ...
  const stats = {
    total: reports.length, success: reports.filter(r => r.result === 'success').length,
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 bg-[#20293F] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-semibold text-white mb-3">月度辅助报告趋势</h3>
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={monthlyTrend}>
              ...
              <Line yAxisId="right" type="monotone" dataKey="improvement" stroke="#22C55E" strokeWidth={2} dot={{ fill: '#22C55E' }} name="改进%" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      ...
      <div className="bg-[#20293F] border border-[#2A354D] rounded-lg p-4">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <div>
            <h2 className="text-lg font-semibold text-white">作业综合辅助任务报告</h2>
            <p className="text-xs text-slate-500 mt-1">AI 辅助报告，含质量评分、AI 建议采纳率、改进度</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button className="flex items-center gap-1.5 px-3 py-1.5 bg-[#2A354D] hover:bg-[#364360] text-slate-300 text-sm rounded-md"><Printer className="w-3.5 h-3.5" />打印</button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              return (
                <div key={r.id} onClick={() => setSelectedId(r.id)}
                  ...
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className={`text-[10px] px-1.5 py-0.5 border rounded ${gradeColor[r.grade]}`}>{r.grade}</span>
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-slate-500">
                    ...
                    <span>·</span>
                    <span>AI <span className="text-blue-300">{r.aiSuggestions}</span></span>
                    <span>·</span>
                    <span>采纳 <span className="text-green-300">{r.appliedSuggestions}</span></span>
                    <span>·</span>
        ...
        {selected ? (
          <div className="bg-[#20293F] border border-[#2A354D] rounded-lg p-4 space-y-3">
            ...
            <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded p-3 text-center">
            ...
            <div className="flex gap-2">
              <button className="flex-1 px-3 py-2 bg-[#2A354D] hover:bg-[#364360] text-slate-300 text-xs rounded-md flex items-center justify-center gap-1.5"><Eye className="w-3 h-3" />查看</button>
...
export default JobAssistantTaskReport;
