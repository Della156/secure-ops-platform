...
import { Download, Calendar, FileText, Briefcase } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from 'recharts';

const chartData = [
  { name: '同步', total: 20, success: 19 },
  { name: '清理', total: 15, success: 14 },
  { name: '备份', total: 18, success: 18 },
  { name: '扫描', total: 10, success: 9 },
  { name: '升级', total: 5, success: 3 },
];
...
const mockReportData: ReportItem[] = [
  { category: '同步作业', total: 50, success: 48, rate: 96 },
  { category: '清理作业', total: 40, success: 38, rate: 95 },
  { category: '备份作业', total: 35, success: 35, rate: 100 },
  { category: '扫描作业', total: 25, success: 23, rate: 92 },
];
...
export function JobReport() {
  ...
  const stats = {
    totalJobs: mockReportData.reduce((sum, d) => sum + d.total, 0),
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">作业报告</h2>
        <p className="text-sm text-gray-400 mt-1">作业执行的综合报告</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          ...
          <p className="text-2xl font-semibold text-white mt-1">{stats.totalJobs}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">作业类型统计</h3>
              ...
              <div key={item.category} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-32 bg-[#111827] rounded-full h-2">
                    <div className={`h-2 rounded-full ${item.rate >= 95 ? 'bg-green-500' : item.rate >= 90 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${item.rate}%` }} />
                  </div>
                  <span className={`text-sm font-medium w-12 text-right ${item.rate >= 95 ? 'text-green-400' : item.rate >= 90 ? 'text-yellow-400' : 'text-red-400'}`}>
                    {item.rate}%
                  </span>
                </div>
              </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2A354D]">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">作业类型</th>
                ...
                <tr key={item.category} className="border-b border-[#2A354D] hover:bg-[#2A354D]/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={`text-sm font-medium ${item.rate >= 95 ? 'text-green-400' : item.rate >= 90 ? 'text-yellow-400' : 'text-red-400'}`}>{item.rate}%</span>
                    </div>
                  </td>
                </tr>
              ))}
