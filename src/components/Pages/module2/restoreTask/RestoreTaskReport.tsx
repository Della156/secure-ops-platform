...
const restoreData = [
  { date: '05-28', count: 8, success: 6 },
  { date: '05-29', count: 12, success: 11 },
  { date: '05-30', count: 6, success: 5 },
  { date: '05-31', count: 10, success: 9 },
  { date: '06-01', count: 14, success: 12 },
  { date: '06-02', count: 8, success: 6 },
];

const durationData = [
  { date: '05-28', avgDuration: 35, maxDuration: 60 },
  { date: '05-29', avgDuration: 30, maxDuration: 55 },
  { date: '05-30', avgDuration: 40, maxDuration: 70 },
  { date: '05-31', avgDuration: 28, maxDuration: 50 },
  { date: '06-01', avgDuration: 32, maxDuration: 58 },
  { date: '06-02', avgDuration: 38, maxDuration: 65 },
];

export function RestoreTaskReport() {
  const [dateRange, setDateRange] = useState({ start: '2026-06-01', end: '2026-06-02' });

  const stats = {
    totalTasks: restoreData.reduce((sum, d) => sum + d.count, 0),
    totalSuccess: restoreData.reduce((sum, d) => sum + d.success, 0),
    avgSuccessRate: Math.round(restoreData.reduce((sum, d) => sum + (d.success / d.count * 100), 0) / restoreData.length),
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">恢复任务报告</h2>
        <p className="text-sm text-gray-400 mt-1">时间段内恢复任务统计分析（成功率、耗时）、恢复报告导出</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">恢复任务统计</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={restoreData}>
        ...
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">恢复耗时趋势</h3>
          <ResponsiveContainer width="100%" height={250}>
