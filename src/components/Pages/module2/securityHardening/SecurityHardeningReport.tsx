...
const chartData = [
  { name: '账户安全', total: 100, completed: 95 },
  { name: '访问控制', total: 100, completed: 88 },
  { name: '数据加密', total: 100, completed: 92 },
  { name: '审计日志', total: 100, completed: 85 },
  { name: '系统加固', total: 100, completed: 90 },
];
...
const mockReportData: ReportItem[] = [
  { category: '账户安全加固', total: 50, completed: 48, rate: 96 },
  { category: '访问控制加固', total: 40, completed: 35, rate: 87.5 },
  { category: '数据加密加固', total: 30, completed: 28, rate: 93.3 },
  { category: '审计日志配置', total: 25, completed: 21, rate: 84 },
];

export function SecurityHardeningReport() {
  const [reportDate, setReportDate] = useState('2026-06-02');

  const stats = {
    totalTasks: mockReportData.reduce((sum, d) => sum + d.total, 0),
    totalCompleted: mockReportData.reduce((sum, d) => sum + d.completed, 0),
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">安全加固报告</h2>
        <p className="text-sm text-gray-400 mt-1">安全加固的综合报告</p>
      </div>
        ...
        <div className="bg-[#1E2736] border border-green-500/30 rounded-lg p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xl font-semibold text-green-400">{stats.totalCompleted}</p>
            </div>
          </div>
        </div>
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <p className="text-gray-400 text-sm">加固覆盖率</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">加固类型统计</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={chartData}>
              ...
              <Legend />
              <Bar dataKey="total" name="目标" fill="#6B7280" radius={[4, 4, 0, 0]} />
        ...
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">加固率统计</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2A354D]">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">加固类型</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">总数</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">已完成</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">加固率</th>
              </tr>
            </thead>
            <tbody>
              {mockReportData.map((item) => (
                <tr key={item.category} className="border-b border-[#2A354D] hover:bg-[#2A354D]/50">
                  ...
                  <td className="px-4 py-3 text-sm text-green-400">{item.completed}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
