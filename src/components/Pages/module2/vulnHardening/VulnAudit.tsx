...
import { Search, Download, Calendar, User, AlertCircle, CheckCircle, XCircle } from 'lucide-react';
...
const mockAudit: AuditItem[] = [
  { id: 'AUD-001', operator: 'admin', vulnId: 'CVE-2024-0001', target: 'prod-server-01', action: '执行修复', time: '2026-06-02 10:00:00', result: 'success' },
  { id: 'AUD-002', operator: 'admin', vulnId: 'CVE-2024-0002', target: 'prod-db', action: '执行修复', time: '2026-06-02 09:30:00', result: 'success' },
];

export function VulnAudit() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">安全漏洞加固审计</h2>
        <p className="text-sm text-gray-400 mt-1">漏洞加固操作的详细审计日志记录（操作人、时间、对象、漏洞），日志查询与导出</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#111827]">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">漏洞编号</th>
              ...
              <tr key={item.id} className="hover:bg-[#2A354D]/30">
                ...
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-red-400">{item.vulnId}</span>
                  </div>
                </td>
