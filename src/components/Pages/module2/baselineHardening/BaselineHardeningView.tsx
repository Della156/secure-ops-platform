...
import { Search, Filter, Download, Calendar, CheckCircle, XCircle, Clock, Shield } from 'lucide-react';
...
const mockTasks: HardeningTask[] = [
  { id: 'HARD-001', name: '系统基线加固', target: 'prod-server-01', strategy: '标准加固策略', status: 'completed', hardeningTime: '2026-06-02 10:00:00', result: '加固成功' },
  { id: 'HARD-002', name: '数据库基线加固', target: 'prod-db', strategy: '数据库加固策略', status: 'completed', hardeningTime: '2026-06-02 09:30:00', result: '加固成功' },
  { id: 'HARD-003', name: '网络设备加固', target: 'fw-01', strategy: '网络设备策略', status: 'failed', hardeningTime: '2026-06-02 09:00:00', result: '加固失败：连接超时' },
  { id: 'HARD-004', name: '应用服务器加固', target: 'app-01', strategy: '标准加固策略', status: 'pending', hardeningTime: '2026-06-02 08:30:00' },
];

export function BaselineHardeningView() {
  const [tasks] = useState(mockTasks);
  const [searchKeyword, setSearchKeyword] = useState('');
  const [statusFilter, setStatusFilter] = useState('all');

  const filteredTasks = tasks.filter(task => {
    const matchSearch = !searchKeyword || task.target.includes(searchKeyword) || task.strategy.includes(searchKeyword);
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">安全基线加固视图</h2>
        <p className="text-sm text-gray-400 mt-1">加固任务列表展示、加固过程展示、加固结果展示、条件查询、数据导出、数据统计</p>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-[#111827]">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400">加固时间</th>
          ...
          <tbody className="divide-y divide-[#2A354D]">
            {filteredTasks.map((task) => (
              <tr key={task.id} className="hover:bg-[#2A354D]/30">
                ...
                <td className="px-4 py-4 text-sm text-gray-300">{task.strategy}</td>
                <td className="px-4 py-4">
                  <div className="flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-gray-500" />
                    <span className="text-sm text-gray-400">{task.hardeningTime}</span>
                  </div>
                </td>
                <td className="px-4 py-4">
