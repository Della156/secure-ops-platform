...
import { Activity, CheckCircle, Clock, AlertTriangle, Settings } from 'lucide-react';
...
const mockTasks: MonitorTask[] = [
  ...
  { 
    ...
    params: [
      ...
      { name: 'log-level', oldValue: 'info', newValue: 'warning' },
    ],
    startTime: '2026-06-02 09:30:00'
  },
];

export function HardeningStatusMonitor() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-white">安全基线加固任务状态监控</h2>
        <p className="text-sm text-gray-400 mt-1">加固任务执行状态的实时监控，参数变更对比</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#1E2736] border border-[#2A354D] rounded-lg p-4">
          <h3 className="text-sm font-medium text-gray-300 mb-4">加固任务列表</h3>
          <div className="space-y-3">
            {tasks.map((task) => (
              ...
              >
                ...
                <div className="text-xs text-gray-400 mb-2">目标: {task.target}</div>
                {task.status === 'running' && (
                  <div>
                    <div className="w-full bg-[#2A354D] rounded-full h-1">
                      <div className="h-1 rounded-full bg-blue-500" style={{ width: `${task.progress}%` }} />
                    </div>
                    <p className="text-xs text-blue-400 mt-1">{Math.round(task.progress)}%</p>
                  </div>
                )}
              </div>
            ))}
