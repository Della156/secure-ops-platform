...
const reports = [
  { id: 'REP-001', name: '2026年6月资产确认周报', type: '周报', generatedAt: '2026-06-03 08:30', status: 'generated', confirmed: 521, total: 543, format: 'PDF' },
  { id: 'REP-002', name: '2026年5月资产确认月报', type: '月报', generatedAt: '2026-06-01 00:00', status: 'generated', confirmed: 512, total: 530, format: 'PDF' },
  { id: 'REP-003', name: '数据中心资产确认报告', type: '专项报告', generatedAt: '2026-06-03 09:45', status: 'generated', confirmed: 245, total: 250, format: 'PDF' },
];

const stats = {
  totalReports: reports.length,
  generatedReports: reports.filter(r => r.status === 'generated').length,
  avgConfirmRate: Math.round(((521 + 512 + 245) / (543 + 530 + 250)) * 100),
  totalConfirmed: reports.reduce((sum, r) => sum + r.confirmed, 0),
};

export function AssetConfirmationReport() {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="资产确认任务报告" description="生成和管理资产确认任务报告"
        actions={[
      ...
      <StatsCardGrid cols={4}>
        ...
        <StatsCard title="平均确认率" value={`${stats.avgConfirmRate}%`} icon={<BarChart3 className="w-5 h-5" />} color="blue" />
        <StatsCard title="确认资产数" value={stats.totalConfirmed} icon={<AlertTriangle className="w-5 h-5" />} color="yellow" />
      </StatsCardGrid>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-400 mt-1">
                        {report.generatedAt} | 确认率: {Math.round((report.confirmed / report.total) * 100)}%
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#20293F] border border-[#2A354D] rounded-lg p-4">
            <h4 className="text-sm font-semibold text-white mb-3">确认趋势</h4>
            <div className="space-y-2">
              {[
                { month: '1月', rate: 85 },
                { month: '2月', rate: 88 },
...
export default AssetConfirmationReport;