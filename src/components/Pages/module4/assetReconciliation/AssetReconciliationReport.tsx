...
const reports = [
  { id: 'REP-001', name: '2026年6月资产核对周报', type: '周报', generatedAt: '2026-06-03 08:30', status: 'generated', matched: 521, total: 543, format: 'PDF' },
  { id: 'REP-002', name: '2026年5月资产核对月报', type: '月报', generatedAt: '2026-06-01 00:00', status: 'generated', matched: 512, total: 530, format: 'PDF' },
  { id: 'REP-003', name: '数据中心资产核对报告', type: '专项报告', generatedAt: '2026-06-03 09:45', status: 'generated', matched: 245, total: 250, format: 'PDF' },
  { id: 'REP-004', name: '2026年Q2资产核对季报', type: '季报', generatedAt: '2026-07-01 00:00', status: 'generating', matched: 0, total: 0, format: '-' },
];

const stats = {
  totalReports: reports.length,
  generatedReports: reports.filter(r => r.status === 'generated').length,
  avgMatchRate: Math.round(((521 + 512 + 245) / (543 + 530 + 250)) * 100),
  totalReconciled: reports.filter(r => r.status === 'generated').reduce((sum, r) => sum + r.total, 0),
};

export function AssetReconciliationReport() {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="资产核对任务报告" description="生成和管理资产核对任务报告"
        actions={[
      ...
      <StatsCardGrid cols={4}>
        ...
        <StatsCard title="平均匹配率" value={`${stats.avgMatchRate}%`} icon={<BarChart3 className="w-5 h-5" />} color="blue" />
        <StatsCard title="核对资产数" value={stats.totalReconciled} icon={<AlertTriangle className="w-5 h-5" />} color="yellow" />
      </StatsCardGrid>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-400 mt-1">
                        {report.generatedAt} {report.status === 'generated' && `| 匹配率: ${Math.round((report.matched / report.total) * 100)}%`}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#20293F] border border-[#2A354D] rounded-lg p-4">
            <h4 className="text-sm font-semibold text-white mb-3">匹配趋势</h4>
            <div className="space-y-2">
              {[
                { month: '1月', rate: 85 },
                { month: '2月', rate: 88 },
                ...
                <div key={item.month}>
                  ...
                  <div className="h-2 bg-[#111625] rounded-full overflow-hidden">
                    <div className="h-full bg-blue-500 rounded-full" style={{ width: `${item.rate}%` }} />
                  </div>
                </div>
              ))}
            </div>
...
export default AssetReconciliationReport;