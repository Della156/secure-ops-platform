...
export function EndpointAccessLog() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-lg font-semibold text-[#F3F4F6] mb-4">终端访问日志查询</h1>
        <p className="text-[#9CA3AF]">查看和分析终端的访问操作记录</p>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-[#181F32]/50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">目标终端</th>
          ...
          <tbody className="divide-y divide-[#2A354D]">
            {filteredLogs.map(log => (
              <tr key={log.id} className="hover:bg-[#181F32]/30 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-[#D1D5DB]">{log.targetEndpoint}</td>
          ...
          <div className="bg-[#20293F] border border-[#2A354D] rounded-xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#9CA3AF] mb-1">目标终端</label>
                  <p className="text-[#F3F4F6]">{selectedLog.targetEndpoint}</p>
                </div>
                <div>
