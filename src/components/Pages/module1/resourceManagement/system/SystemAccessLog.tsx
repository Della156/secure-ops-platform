...
const mockLogs: AccessLog[] = [
  { id: 'log-1', timestamp: '2026-06-01 10:30:45', user: '张三', action: '登录', targetSystem: 'SIEM日志分析平台', resource: '管理界面', result: 'success', details: '用户成功登录系统', ipAddress: '192.168.1.100' },
  { id: 'log-2', timestamp: '2026-06-01 10:28:12', user: '李四', action: '查询安全事件', targetSystem: '威胁情报平台', resource: 'API接口', result: 'success', details: '查询威胁情报数据', ipAddress: '192.168.1.101' },
  { id: 'log-3', timestamp: '2026-06-01 10:15:33', user: '王五', action: '导出报告', targetSystem: '漏洞扫描系统', resource: '报告功能', result: 'warning', details: '导出大量漏洞报告', ipAddress: '192.168.1.102' },
  { id: 'log-4', timestamp: '2026-06-01 09:45:21', user: '赵六', action: '登录', targetSystem: '用户行为分析', resource: '管理界面', result: 'failed', details: '密码错误', ipAddress: '192.168.1.103' },
  { id: 'log-5', timestamp: '2026-06-01 09:30:00', user: '系统', action: '自动备份', targetSystem: 'SIEM日志分析平台', resource: '数据库', result: 'success', details: '定时备份任务执行完成', ipAddress: '127.0.0.1' },
];

export function SystemAccessLog() {
  ...
  const filteredLogs = logs.filter(log => {
    ...
    const matchSystem = !filterSystem || log.targetSystem === filterSystem;
    return matchSearch && matchUser && matchResult && matchSystem;
  });

  const users = [...new Set(logs.map(l => l.user))];
  const systems = [...new Set(logs.map(l => l.targetSystem))];
  ...
  const handleExport = () => {
    const csv = [
      ['时间', '用户', '动作', '目标系统', '资源', '结果', '详情', 'IP地址'].join(','),
      ...filteredLogs.map(log => [
        log.timestamp, log.user, log.action, log.targetSystem, log.resource, log.result, log.details, log.ipAddress
      ].map(field => `"${field}"`).join(','))
    ].join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = `system_access_logs_${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  };
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-2">系统访问日志查询</h1>
        <p className="text-slate-400">查看和分析安全系统的访问操作记录</p>
      </div>
        ...
        <table className="w-full">
          <thead className="bg-slate-800/50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">目标系统</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {filteredLogs.map(log => (
              <tr key={log.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{log.targetSystem}</td>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-1">目标系统</label>
                  <p className="text-white">{selectedLog.targetSystem}</p>
                </div>
                <div>
