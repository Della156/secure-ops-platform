...
const mockPolicies: AuthPolicy[] = [
  { id: 'policy-1', name: '防火墙管理员访问', target: '主防火墙-FW-01', type: 'role', status: 'active', validFrom: '2026-01-01', validUntil: '2026-12-31' },
  { id: 'policy-2', name: 'IDS监控访问', target: '入侵检测系统-IDS-01', type: 'user', status: 'active', validFrom: '2026-03-15', validUntil: '2026-09-15' },
  { id: 'policy-3', name: 'WAF配置访问', target: 'Web应用防火墙-WAF-01', type: 'role', status: 'inactive', validFrom: '2026-02-01', validUntil: '2026-06-01' },
];

const mockRequests: AuthRequest[] = [
  { id: 'req-1', applicant: '张三', target: '主防火墙-FW-01', reason: '需要进行规则配置更新', status: 'pending', requestTime: '2026-06-01 09:30:00' },
  { id: 'req-2', applicant: '李四', target: '入侵检测系统-IDS-01', reason: '查看告警日志', status: 'approved', requestTime: '2026-05-31 14:20:00' },
  { id: 'req-3', applicant: '王五', target: '终端安全管理-EDR-01', reason: '测试新规则', status: 'rejected', requestTime: '2026-05-30 11:15:00' },
];

export function DeviceAuthManagement() {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-white mb-2">设备访问授权管理</h1>
        <p className="text-slate-400">管理设备访问授权策略，处理授权申请</p>
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 mb-4">
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <h3 className="text-white font-semibold">授权策略列表</h3>
          <button
        ...
        <table className="w-full">
          <thead className="bg-slate-800/50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">授权类型</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {policies.map(policy => (
              <tr key={policy.id} className="hover:bg-slate-800/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-white font-medium">{policy.name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{policy.target}</td>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800">
          <h3 className="text-white font-semibold">授权申请审批</h3>
        </div>
        <table className="w-full">
          <thead className="bg-slate-800/50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">目标设备</th>
              <th className="px-6 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">申请原因</th>
          ...
          <tbody className="divide-y divide-slate-800">
            {requests.map(req => (
              <tr key={req.id} className="hover:bg-slate-800/30 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm text-white font-medium">{req.applicant}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-300">{req.target}</td>
                <td className="px-6 py-4 text-sm text-slate-300 max-w-xs truncate">{req.reason}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-400">{req.requestTime}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-slate-900 border border-slate-800 rounded-xl w-full max-w-lg mx-4">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">目标设备</label>
                <select
                ...
                >
                  ...
                  <option value="主防火墙-FW-01">主防火墙-FW-01</option>
                  <option value="入侵检测系统-IDS-01">入侵检测系统-IDS-01</option>
                  <option value="Web应用防火墙-WAF-01">Web应用防火墙-WAF-01</option>
                  <option value="终端安全管理-EDR-01">终端安全管理-EDR-01</option>
                </select>
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1.5">有效期从</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-1.5">有效期至</label>
                  <input
                    type="date"
