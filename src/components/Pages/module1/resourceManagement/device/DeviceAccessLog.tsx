...
export function DeviceAccessLog() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-lg font-semibold text-[#F3F4F6] mb-4">设备访问日志查询</h1>
        <p className="text-[#9CA3AF]">查看和分析设备的访问操作记录</p>
      </div>
          ...
          <tbody className="divide-y divide-[#2A354D]">
            {filteredLogs.map(log => (
              <tr key={log.id} className="hover:bg-[#181F32]/30 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-[#D1D5DB]">{log.targetDevice}</td>
      ...
      {selectedLog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-[#20293F] border border-[#2A354D] rounded-xl w-full max-w-lg">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-[#9CA3AF] mb-1">目标设备</label>
                  <p className="text-[#F3F4F6]">{selectedLog.targetDevice}</p>
                </div>
                <div>
