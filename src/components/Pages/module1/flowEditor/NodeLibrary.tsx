...
const mockNodes: NodeItem[] = [
  {
    ...
    usageCount: 45,
    icon: <Shield className="w-5 h-5 text-[#00C853]" />,
    inputs: [
  ...
  },
  {
    ...
    usageCount: 28,
    icon: <Database className="w-5 h-5 text-[#FF9100]" />,
    inputs: [
  ...
  },
  {
    ...
    usageCount: 0,
    icon: <Cloud className="w-5 h-5 text-[#FF9100]" />,
    inputs: [
  ...
  },
  {
    ...
    usageCount: 89,
    icon: <Terminal className="w-5 h-5 text-[#00BCD4]" />,
    inputs: [
  ...
  },
  {
    ...
    usageCount: 12,
    icon: <Database className="w-5 h-5 text-[#FF3B30]" />,
    inputs: [],
    outputs: [],
  },
];
...
export function NodeLibrary() {
  ...
  return (
    <div>
      {/* 页面标题 */}
      <div className="mb-6">
        <h1 className="text-lg font-semibold text-[#F3F4F6] mb-4">自动化能力节点库</h1>
        <p className="text-[#9CA3AF]">浏览和管理可用于流程编排的自动化节点</p>
      </div>
      ...
      <div className="bg-[#20293F] border border-[#2A354D] rounded-xl p-4 mb-4">
        <div className="flex flex-wrap gap-4 items-center justify-between">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 pr-4 py-2 bg-[#181F32] border border-[#2A354D] rounded-lg text-[#F3F4F6] placeholder-[#6B7280] focus:outline-none focus:ring-2 focus:ring-[#0066FF] w-80"
              />
            </div>
          ...
          <div className="flex items-center gap-3">
            {/* 视图切换 */}
            <div className="flex items-center gap-1 bg-[#181F32] rounded-lg p-0.5">
              <button
        ...
        <div className="mt-4 flex flex-wrap gap-2 items-center">
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-full text-xs border transition-colors ${
              !selectedTag
                ? 'bg-[#0066FF]/20 text-[#0066FF] border-blue-500/30'
                : 'bg-[#181F32] text-[#9CA3AF] border-[#2A354D] hover:border-[#3A4560]'
            }`}
          >
            全部
          </button>
          ...
          <div className="bg-[#20293F] border border-[#2A354D] rounded-xl p-4 sticky top-8">
            ...
            <div className="space-y-1">
              ...
              >
                ...
                <span className="text-[#6B7280]">{nodes.length}</span>
              </button>
              {mockCategories.map(cat => (
                <button
                  key={cat.id}
        ...
        <div className="flex-1">
          ...
          <div className="mb-4 flex items-center justify-between">
            <p className="text-[#9CA3AF] text-sm">
              找到 <span className="text-[#F3F4F6] font-medium">{filteredNodes.length}</span> 个节点
            </p>
          </div>
          ...
          {viewMode === 'grid' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {filteredNodes.map(node => (
                <div
                  key={node.id}
                  className="bg-[#20293F] border border-[#2A354D] rounded-xl p-4 hover:border-[#2A354D] transition-colors cursor-pointer"
                ...
                >
                  ...
                  <h3 className="text-[#F3F4F6] font-medium mb-1">{node.name}</h3>
                  <p className="text-[#9CA3AF] text-sm mb-3 line-clamp-2">{node.description}</p>
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {node.tags.slice(0, 3).map(tag => (
                      <span key={tag} className="px-2 py-0.5 bg-[#181F32] text-[#9CA3AF] rounded text-xs">
                        {tag}
                      </span>
                    ))}
                    {node.tags.length > 3 && (
                      <span className="px-2 py-0.5 bg-[#181F32] text-[#6B7280] rounded text-xs">
                        +{node.tags.length - 3}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-3 pt-3 border-t border-[#2A354D] flex gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); handleViewDetail(node); }}
                      className="flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-[#181F32] hover:bg-[#2A354D] text-[#D1D5DB] rounded-lg text-sm transition-colors"
                    >
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-1.5 px-3 py-1.5 bg-[#181F32] hover:bg-[#2A354D] text-[#D1D5DB] rounded-lg text-sm transition-colors"
                    >
            ...
            <div className="bg-[#20293F] border border-[#2A354D] rounded-xl overflow-hidden">
              <table className="w-full">
                <thead className="bg-[#181F32]/50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-[#9CA3AF] uppercase tracking-wider">节点</th>
                ...
                <tbody className="divide-y divide-[#2A354D]">
                  {filteredNodes.map(node => (
                    <tr key={node.id} className="hover:bg-[#181F32]/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="p-1.5 bg-[#181F32] rounded-lg">
                            {node.icon}
                          </div>
                          <div>
                            <div className="text-[#F3F4F6] font-medium text-sm">{node.name}</div>
                            <div className="text-[#6B7280] text-xs truncate max-w-xs">{node.description}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[#D1D5DB] text-sm">{getCategoryName(node.category)}</span>
                      </td>
                      <td className="px-6 py-4">
                        {getStatusBadge(node.status)}
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[#D1D5DB] text-sm">v{node.version}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[#D1D5DB] text-sm">{node.usageCount}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-[#9CA3AF] text-sm">{node.createdAt}</span>
                      </td>
                      <td className="px-6 py-4">
              ...
              {filteredNodes.length === 0 && (
                <div className="px-6 py-12 text-center">
                  <p className="text-[#6B7280]">暂无匹配的节点</p>
                </div>
              )}
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-[#20293F] border border-[#2A354D] rounded-xl w-full max-w-3xl mx-4 max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
              <p className="text-[#D1D5DB] mb-4">{selectedNode.description}</p>
              
              <div className="flex flex-wrap gap-2 mb-6">
                {selectedNode.tags.map(tag => (
                  <span key={tag} className="px-2.5 py-1 bg-[#181F32] text-[#D1D5DB] rounded-full text-xs">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-[#181F32] rounded-lg p-3">
                  <p className="text-[#6B7280] text-xs mb-1">作者</p>
                  <p className="text-[#F3F4F6] font-medium">{selectedNode.author}</p>
                </div>
                <div className="bg-[#181F32] rounded-lg p-3">
                  ...
                  <div className="bg-[#181F32] rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                      <thead className="bg-[#2A354D]/50">
                        <tr>
                          <th className="px-4 py-2 text-left text-[#9CA3AF] font-medium">参数名</th>
                          <th className="px-4 py-2 text-left text-[#9CA3AF] font-medium">类型</th>
                          <th className="px-4 py-2 text-left text-[#9CA3AF] font-medium">必填</th>
                          <th className="px-4 py-2 text-left text-[#9CA3AF] font-medium">描述</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#2A354D]">
                        {selectedNode.inputs.map((input, idx) => (
                          <tr key={idx}>
                            <td className="px-4 py-2 text-[#F3F4F6] font-mono">{input.name}</td>
                            <td className="px-4 py-2 text-[#0066FF]">{input.type}</td>
                            <td className="px-4 py-2">
                              {input.required ? (
                                <span className="text-[#FF3B30] text-xs">是</span>
                              ) : (
                                <span className="text-[#6B7280] text-xs">否</span>
                              )}
                            </td>
                            <td className="px-4 py-2 text-[#9CA3AF]">{input.description}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-[#2A354D]">
                        {selectedNode.outputs.map((output, idx) => (
                          <tr key={idx}>
                            <td className="px-4 py-2 text-[#F3F4F6] font-mono">{output.name}</td>
                            <td className="px-4 py-2 text-[#00C853]">{output.type}</td>
                            <td className="px-4 py-2 text-[#9CA3AF]">{output.description}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-[#20293F] border border-[#2A354D] rounded-xl w-full max-w-lg mx-4">
            <div className="flex items-center justify-between p-4 border-b border-[#2A354D]">
              <h3 className="text-lg font-semibold text-[#F3F4F6]">新建节点</h3>
              <button
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[#D1D5DB] mb-1.5">分类</label>
