...
/* --- Mock Data --- */
const mockData: Item[] = [
{ id:'SJ-001', name:'恶意文档研判', status:'running', sampleType:'Office文档', analysisResult:'检测中', confidence:0, submitTime:'2026-06-02 08:00' },
  { id:'SJ-002', name:'可疑PE文件研判', status:'completed', sampleType:'可执行文件', analysisResult:'恶意', confidence:95, submitTime:'2026-06-01 08:00' },
  { id:'SJ-003', name:'钓鱼邮件研判', status:'running', sampleType:'邮件附件', analysisResult:'检测中', confidence:0, submitTime:'2026-06-02 10:00' },
  { id:'SJ-004', name:'恶意脚本研判', status:'failed', sampleType:'JavaScript', analysisResult:'分析失败', confidence:0, submitTime:'2026-06-02 06:00' },
  { id:'SJ-005', name:'可疑URL研判', status:'running', sampleType:'URL链接', analysisResult:'可疑', confidence:72, submitTime:'2026-06-02 14:00' }
];
...
export function SampleJudgmentView() {
  ...
  return (
    <div>
      <PageHeader title="深度样本研判视图" description="多通道样本采集与深度研判分析，自动调用分析工具"
        actions={[
  ...
  { key:'sampleType', title:'样本类型' },
  { key:'analysisResult', title:'分析结果', render:(v) => <StatusBadge status={v==='恶意'?'failed':v==='可疑'?'warning':v==='检测中'?'running':'info'} /> },
  { key:'confidence', title:'置信度', sortable:true, render:(v) => v>0 ? <span className='text-green-400'>{v}%</span> : <span className='text-gray-500'>-</span> },
