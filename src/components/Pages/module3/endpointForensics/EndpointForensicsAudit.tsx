...
const AUDITS: EndpointAudit[] = [
  { id: 'EFA-2026-401', taskId: 'EFH-20260604001', taskName: 'HOST-FIN-002 证据采集', auditType: 'evidence', status: 'reviewing', operator: '取证-张工', operatorIp: '10.20.30.45', reviewer: '李娜', auditTime: '2026-06-04 11:00:00', riskLevel: 'high', finding: '证据完整性校验通过，等待司法鉴定复核', evidenceChain: ['磁盘镜像 hash', 'MD5/SHA256 校验', '取证工具日志', '操作员审计'] },
  { id: 'EFA-2026-400', taskId: 'EFH-20260603007', taskName: 'HOST-DEV-022 USB 取证', auditType: 'evidence', status: 'approved', operator: '取证-李工', operatorIp: '10.20.30.45', reviewer: '李娜', approver: '王刚', auditTime: '2026-06-03 21:00:00', riskLevel: 'high', finding: 'USB 使用记录完整提取，已用于事件分析', evidenceChain: ['USB 历史记录', '注册表键值', '文件操作日志'] },
  { id: 'EFA-2026-399', taskId: 'EFH-20260602007', taskName: 'HOST-APP-005 进程取证', auditType: 'chain', status: 'approved', operator: '取证-王工', operatorIp: '10.20.30.45', reviewer: '李娜', approver: '王刚', auditTime: '2026-06-02 18:00:00', riskLevel: 'critical', finding: '可疑进程已提取 156 个制品，证据链完整', evidenceChain: ['进程列表', 'DLL 列表', '网络连接', '文件句柄', '注册表修改'] },
  { id: 'EFA-2026-398', taskId: 'EFH-20260603006', taskName: 'HOST-APP-005 注册表取证', auditType: 'evidence', status: 'archived', operator: '系统自动', operatorIp: '10.20.30.10', reviewer: '李娜', auditTime: '2026-06-03 19:00:00', riskLevel: 'medium', finding: '注册表 hive 已归档存储', evidenceChain: ['SYSTEM hive', 'SOFTWARE hive', 'NTUSER.DAT'] },
  { id: 'EFA-2026-397', taskId: 'EFH-20260602008', taskName: 'HOST-FIN-002 网络取证', auditType: 'compliance', status: 'rejected', operator: '取证-张工', operatorIp: '10.20.30.45', reviewer: '李娜', approver: '王刚', auditTime: '2026-06-02 15:00:00', riskLevel: 'high', finding: '证据采集超时，部分数据不完整，已重做', evidenceChain: ['网络会话日志'] },
  { id: 'EFA-2026-396', taskId: 'EFH-20260601009', taskName: '全网可疑进程扫描', auditType: 'access', status: 'approved', operator: '系统自动', operatorIp: '10.20.30.10', reviewer: '李娜', approver: '王刚', auditTime: '2026-06-02 00:00:00', riskLevel: 'medium', finding: '全网扫描结果已存档', evidenceChain: ['扫描报告', '可疑进程清单'] },
  { id: 'EFA-2026-395', taskId: 'EFH-20260604003', taskName: 'HOST-APP-005 内存取证', auditType: 'evidence', status: 'pending', operator: '取证-王工', operatorIp: '10.20.30.45', auditTime: '2026-06-04 10:00:00', riskLevel: 'high', finding: '内存取证失败，等待重试', evidenceChain: ['失败日志'] },
];

const TYPE_LABEL: Record<EndpointAudit['auditType'], { label: string; color: string }> = {
...
const COLUMNS = [
  { key: 'id', title: '审计 ID', width: '130px', render: (r: EndpointAudit) => <span className="font-mono text-xs text-slate-400">{r.id}</span> },
  { key: 'taskName', title: '取证任务', render: (r: EndpointAudit) => <div><p className="text-sm text-slate-100">{r.taskName}</p><p className="text-[10px] text-slate-500">{r.taskId}</p></div> },
  { key: 'type', title: '类型', width: '90px', render: (r: EndpointAudit) => <span className={`text-xs px-1.5 py-0.5 rounded ${TYPE_LABEL[r.auditType].color}`}>{TYPE_LABEL[r.auditType].label}</span> },
  { key: 'status', title: '状态', width: '80px', render: (r: EndpointAudit) => <StatusBadge status={STATUS_MAP[r.status].status} /> },
  { key: 'risk', title: '风险', width: '70px', render: (r: EndpointAudit) => <StatusBadge status={RISK_MAP[r.riskLevel].status} /> },
  { key: 'operator', title: '操作人', width: '100px', render: (r: EndpointAudit) => <span className="text-xs text-slate-300">{r.operator}</span> },
  { key: 'auditTime', title: '审计时间', width: '150px', render: (r: EndpointAudit) => <span className="text-xs text-slate-400 font-mono">{r.auditTime}</span> },
];
 ...
 */
export function EndpointForensicsAudit() {
...
export default EndpointForensicsAudit;
