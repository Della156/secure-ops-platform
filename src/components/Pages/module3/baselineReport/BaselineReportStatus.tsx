...
const tasks = [
  { id: 'TS-001', name: '主机安全基线检查', status: 'running', progress: 75, duration: '00:45:30', pushStatus: 'pending' },
  { id: 'TS-002', name: '网络安全基线检查', status: 'completed', progress: 100, duration: '02:15:00', pushStatus: 'success' },
  { id: 'TS-003', name: '数据库基线检查', status: 'failed', progress: 30, duration: '00:12:00', pushStatus: 'failed' },
  { id: 'TS-004', name: '应用安全基线检查', status: 'pending', progress: 0, duration: '-', pushStatus: 'pending' },
  { id: 'TS-005', name: '系统安全基线检查', status: 'running', progress: 60, duration: '01:30:00', pushStatus: 'pending' },
];

const statusConfig = {
  pending: { label: '待执行', color: 'text-slate-400', bg: 'bg-slate-500/20', icon: <Clock className="w-4 h-4" /> },
  running: { label: '执行中', color: 'text-blue-400', bg: 'bg-blue-500/20', icon: <div className="w-4 h-4 border-2 border-blue-400 border-t-transparent rounded-full animate-spin" /> },
...
export function BaselineReportStatus() {
      ...
      setTaskList(prev => prev.map(task => {
        if (task.status === 'running' && task.progress < 100) {
          ...
          return {
            ...
            pushStatus: newProgress === 100 ? 'success' : 'pending',
          };
        }
        return task;
      }));
  ...
  const handleRetry = (taskId: string) => {
    setTaskList(prev => prev.map(task =>
      task.id === taskId ? { ...task, status: 'running', progress: 0, pushStatus: 'pending' } : task
    ));
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="基线防护报告任务状态监控" description="实时监控基线报告任务的生成和推送状态"
        actions={[
            ...
            <div key={task.id} className="bg-[#20293F] border border-[#2A354D] rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between mt-3 pt-3 border-t border-[#2A354D]">
                  ...
                  {task.duration !== '-' && (
                    <span className="text-xs text-slate-500">耗时: {task.duration}</span>
                  )}
                </div>

                <div className="flex items-center justify-between mt-3">
                  <span className="text-xs text-slate-500">已耗时: {task.duration}</span>
                  <div className="flex items-center gap-2">
                    {task.status === 'pending' && (
