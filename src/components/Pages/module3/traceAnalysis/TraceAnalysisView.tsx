...
interface Item {
  ...
  analyzing: number;
  completed: number;
  attackersFound: number;
}

/* --- Mock Data --- */
const mockData: Item[] = [
{ id:'TA-001', name:'核心网告警溯源分析', status:'running', totalAlerts:12, analyzing:3, completed:7, attackersFound:2 },
  { id:'TA-002', name:'横向渗透溯源分析', status:'completed', totalAlerts:8, analyzing:0, completed:8, attackersFound:1 },
  { id:'TA-003', name:'数据窃取溯源分析', status:'running', totalAlerts:15, analyzing:5, completed:8, attackersFound:3 },
  { id:'TA-004', name:'APT攻击溯源分析', status:'failed', totalAlerts:20, analyzing:2, completed:12, attackersFound:0 },
  { id:'TA-005', name:'钓鱼攻击溯源分析', status:'running', totalAlerts:6, analyzing:2, completed:3, attackersFound:1 }
];
...
export function TraceAnalysisView() {
  ...
  return (
    <div>
      <PageHeader title="溯源分析视图" description="自动化攻击行为分析与溯源，还原攻击路径，定位攻击者"
        actions={[
  ...
  { key:'totalAlerts', title:'总告警', sortable:true },
  { key:'analyzing', title:'分析中' },
  { key:'completed', title:'已完成', sortable:true },
  { key:'attackersFound', title:'定位攻击者', render:(v) => <span className='text-red-400 font-medium'>{v}</span> },
